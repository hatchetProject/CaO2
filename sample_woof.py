...
def main(args):
    ...
    for c in tqdm(range(len(sel_classes))):
        ...
        if not os.path.exists(os.path.join(args.save_dir, sel_class+"_ss")):
            os.makedirs(os.path.join(args.save_dir, sel_class+"_ss"), exist_ok=True)
        for img_idx, sample in enumerate(samples):
            save_sample = vae.decode(sample.unsqueeze(0) / 0.18215).sample
            save_image(save_sample, os.path.join(args.save_dir, sel_class+"_ss", f"{img_idx}.png"), normalize=True, value_range=(-1, 1))
