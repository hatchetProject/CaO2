...
def main(args):
    ...
    for c in tqdm(range(len(sel_classes))):
        ...
        # Get the correct and most/least confident samples
        ...
        confident_indices = torch.argsort(confidence, descending=False) # ADJUSTABLE
        ...
        if len(result_indices) < batch_size:
            print(f"Not enough correct samples for class {sel_class}.")
        ...
        model.train()
        ts = range(1, args.num_sampling_steps // 8)  # ADJUSTABLE
        train_size = 1
...
def main_var(args):
    ...
    for c in tqdm(range(len(sel_classes))):
        ...
        #     ])
        # class_path = os.path.join(imagenet_train_path, sel_class)
        # image_paths = glob.glob(os.path.join(class_path, '*.jpg'))
        # image_paths = image_paths[:args.ipc]
        # images = [transform_load(Image.open(img_path)) for img_path in image_paths]

        # orig_samples = torch.stack(images).cuda()
        # samples = vae.encode(orig_samples).latent_dist.sample().mul_(0.18215)
        # orig_samples = vae.decode(samples / 0.18215).sample
        # for img_idx, save_sample in enumerate(orig_samples):
        #     save_image(save_sample, os.path.join(args.save_dir, sel_class, f"{img_idx}.png"), normalize=True, value_range=(-1, 1))
